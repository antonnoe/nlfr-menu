...
// Hoeveel van de WordPress-excerpt we bewaren. Zie de toelichting bij het veld.
export const SAMENVATTING_MAX = 300;
...
export function ontHtml(ruw) {
  return String(ruw == null ? "" : ruw)
    // Tag -> SPATIE, niet -> niets. Een titel heeft geen tags, maar een
    // WordPress-excerpt is <p>…</p><p>…</p>: zonder die spatie plakt het einde
    // van de ene alinea aan het begin van de volgende ("aankoopDe koper"), en
    // dat woord overlapt vervolgens nergens mee. Dezelfde fout die bij de
    // verenigingenfeed "NATIONALITEITOp donderdag" opleverde. De samentrekking
    // van witruimte onderaan haalt de extra spaties er weer uit.
    .replace(/<[^>]*>/g, " ")
    ...
    .replace(/\s+/g, " ")
    // De spatie die de tagvervanging hierboven vóór een leesteken kan zetten
    // ("<strong>termijn</strong>." -> "termijn .") weer weghalen. Alleen de
    // Nederlandse leestekens: deze velden zijn Nederlandstalig, en in het Frans
    // hoort er juist wél een spatie vóór ; : ! ?.
    .replace(/ +([.,;:!?…)\]])/g, "$1")
    .trim();
}
...
export function postsUrl(pagina) {
  return `${IF_API_BASIS}/posts?per_page=${IF_PER_PAGINA}&page=${pagina}` +
    `&_fields=id,link,title,excerpt,modified,modified_gmt,categories&orderby=modified&order=desc`;
}
  ...
  for (let pagina = 1; pagina <= IF_MAX_PAGINAS; pagina += 1) {
    ...
    for (const p of lijst) {
      ...
      artikelen.push({
        ...
        // De samenvatting draagt de woordoverlap in lib/ifsuggestie.js. Op
        // SAMENVATTING_MAX afgekapt: de hele index gaat in KV, en met de volle
        // excerpts groeit hij van 70 naar 175 KB. De eerste 300 tekens bevatten
        // het onderwerp; de rest is uitwerking en voegt vooral ruis toe.
        samenvatting: ontHtml(p.excerpt && p.excerpt.rendered).slice(0, SAMENVATTING_MAX),
      });
    }
