...
export function nieuweMeting(nu = Date.now()) {
  return {
    ...
    opgeruimdeRedenen: {},
    gesnoeid: 0,
    // Een storing die de HELE ronde raakt in plaats van één cluster. Blijft
    // null zolang er niets aan de hand is; zie aanroepStoring().
    storing: null,
  };
}

// WAT VOOR FOUT KWAM ER TERUG BIJ DE MODELAANROEP.
// ---------------------------------------------------------------------------
// Op 6 september viel de perssynthese uit. In het draailog stond alleen een
// telling: "mislukt=5". Dat las als vijf clusters die inhoudelijk stukliepen,
// terwijl het vijf keer dezelfde geweigerde API-sleutel was. De reden hoort in
// het log, met de HTTP-status erbij; een telling zonder reden is geen bewaking.
//
// EEN GEWEIGERDE SLEUTEL IS GEEN CLUSTERPROBLEEM. Hij geldt voor elke aanroep
// in de ronde, dus de vier volgende kandidaten leveren gegarandeerd dezelfde
// fout op. Daarom is 401/403 `fataal`: de lus stopt en de ronde meldt het.
//
// 429 en 5xx zijn dat NIET. Die kunnen per aanroep verschillen — een volgende
// cluster kan gewoon slagen — en ze zeggen iets over de drukte bij de dienst,
// niet over deze installatie. Ze krijgen wel hun status in het log, zodat het
// verschil met een inhoudelijke mislukking zichtbaar blijft.
export function aanroepStoring(e) {
  const bericht = e instanceof Error ? e.message : String(e);
  // De SDK gooit een APIError met .status. Sommige omhullingen gebruiken
  // .statusCode; beide worden gelezen voordat er op tekst wordt teruggevallen.
  let status = null;
  if (e && typeof e === "object") {
    if (typeof e.status === "number") status = e.status;
    else if (typeof e.statusCode === "number") status = e.statusCode;
  }
  // Terugval op de tekst. Alleen om een status te VINDEN die er anders niet is;
  // hij kan er nooit een weghalen, dus dit maakt de herkenning ruimer en nooit
  // smaller.
  if (status === null && /\b(401|authentication_error)\b/.test(bericht)) status = 401;
  if (status === null && /\b(403|permission_error)\b/.test(bericht)) status = 403;
  ...
  if (status === 401 || status === 403) {
    return {
      ...
      fataal: true,
      reden:
        `de API-sleutel wordt geweigerd (HTTP ${status}). ANTHROPIC_API_KEY is wél gezet — ` +
        "de cron zou anders al bij de poort zijn gestopt — maar is ongeldig, verlopen of " +
        "van een account zonder toegang tot dit model",
      bericht,
    };
  }
  return { soort: "aanroep", status, fataal: false, reden: bericht, bericht };
}

// Eén regel voor het log, met de status erbij zodra die bekend is.
export function storingRegel(storing, cluster, voorvoegsel = "[pers]") {
  const s = storing || { soort: "aanroep", status: null, reden: "reden onbekend" };
  const wat = s.soort === "sleutel" ? "synthese afgebroken" : "synthese mislukt";
  // Bij een geweigerde sleutel staat de status al in de reden; bij een andere
  // fout hoort hij er alsnog bij, zodra hij bekend is.
  const status = s.soort !== "sleutel" && s.status ? `HTTP ${s.status} — ` : "";
  return `${voorvoegsel} ${wat} voor cluster ${cluster}: ${status}${s.reden}`;
}
...
export function duiding(meting) {
  const m = meting || {};
  // Een geweigerde sleutel overstemt elke andere verklaring: dan is er niets
  // mis met het nieuws, met de zeef of met de drempel.
  if (m.storing && m.storing.soort === "sleutel") {
    return `de ronde is afgebroken: ${m.storing.reden}`;
  }
...
export function logRegels(meting, voorvoegsel = "[pers]") {
  ...
  if (m.storing) {
    uit.push(
      `${voorvoegsel} STORING: ${m.storing.soort}` +
        (m.storing.status ? ` (HTTP ${m.storing.status})` : "") +
        ` — ${m.storing.reden}`
    );
  }
  const gez = m.gezeefd || {};
