...
// De bron die bij een THEMA hoort. Nodig voor aggregaatfeeds: daar draagt het
// item de naam van de vereniging ("LOTgenoten"), niet die van de geconfigureerde
// bron, dus levert opzoeken op naam niets op. Het thema is wél stabiel.
export function bronVoorThema(thema) {
  const t = String(thema || "").trim().toLowerCase();
  if (!t) return null;
  return BRONNEN.find((b) => String(b.thema || "").toLowerCase() === t) || null;
}
