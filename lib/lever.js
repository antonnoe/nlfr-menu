...
export async function lever(req, res, soort) {
  ...
  // ---- BEKEND RISICO: de randcache werkt na bij "Van de site halen" --------
  // Een redactiehandeling maakt de voorgebakken momentopname meteen ongeldig
  // (zie vervalSnapshots in api/review.js), maar de randcache van Vercel houdt
  // dit antwoord daarna nog vast:
  //   s-maxage=900   ongeveer een kwartier vers in de rand;
  //   SWR=86400      daarna mag de rand de OUDE versie nog één keer uitserveren
  //                  terwijl hij op de achtergrond ververst — de eerstvolgende
  //                  lezer na dat kwartier kan het weggehaalde bericht dus nog
  //                  zien, pas de lezer daarna niet meer;
  //   max-age=120    en een browser die de pagina al open had, tot twee minuten.
  // Een weggehaald bericht kan daardoor ongeveer een kwartier bij lezers op het
  // scherm blijven staan. BEWUST GEACCEPTEERD, niet vergeten: een kwartier is de
  // croninterval en daarmee de belofte die deze pagina sowieso doet, en de
  // cachetijd verlagen kost élke lezer snelheid voor één redactiehandeling.
  // Gericht laten vervallen zou een purge-API van het platform vragen (niet
  // gewired, geen token in deze repo) of een extra ongecachet verzoek per
  // paginabezoek om een versiestempel op te halen — dat laatste raakt weer elke
  // lezer. De reviewtool zegt daarom bij die knop hoelang het kan naijlen.
