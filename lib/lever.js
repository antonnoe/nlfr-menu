// De routemechaniek die /api/actueel en /api/actueel-tekst delen.
// ---------------------------------------------------------------------------
// Beide routes doen exact hetzelfde, alleen met een andere helft van het
...
// (live feeds + KV), splitsen, BEIDE leveringen wegschrijven en de gevraagde
// helft antwoorden. Dat "beide wegschrijven" is bewust: wie de ene route mist,
// warmt meteen ook de andere op, zodat de tweede levering niet nóg een keer de
// feeds hoeft op te halen.
//
// Eén module, zodat de cacheheaders en het terugvalgedrag van de twee routes
...
// Een tekst-levering heeft geen `tegels`, dus de vormtoets van snapshotBruikbaar
// past er niet op. Voor de leeftijdstoets doet hij dat wél; daarom krijgt hij
// hier een object mee dat aan de vormeis voldoet.
...
// Handelt één verzoek af. `soort` is "compact" of "tekst".
export async function lever(req, res, soort) {
  ...
  if (bruikbaar(snapshot, nu)) {
    ...
    // Allebei wegschrijven: de andere route heeft dan niets meer te doen.
    try {
