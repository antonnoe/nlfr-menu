...
// concepten van vóór deze versie).
export function afleidKop(tekst, maxWoorden = 8) {
  const zin = eersteZin(tekst, 80).replace(/[.!?…]+$/, "");
  const woorden = zin.split(" ").filter(Boolean);
  const kort = woorden.slice(0, maxWoorden).join(" ");
  return woorden.length > maxWoorden ? `${kort}…` : kort;
}

// Welke pers-tegel hoort bij een publicatie? Brandtrefwoord wint; anders het
// (meest voorkomende) brontthema van de aangehaalde bronnen; standaard landelijk.
export function persTegelVoor(pub, naamNaarThema) {
  const tekstBlob = normaliseer(
    `${pub.kop || ""} ${pub.tekst || ""} ${(pub.bronnen || [])
      .map((b) => b.titel || "")
      .join(" ")}`
  );
