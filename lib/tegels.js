...
function ontdubbelOp(lijst, tekstVan, drempel) {
  const gezien = [];
  const uit = [];
  for (const d of lijst) {
    const t = dedupTokens(tekstVan(d));
    if (gezien.some((g) => dedupJaccard(t, g) >= drempel)) continue;
    gezien.push(t);
    uit.push(d);
  }
  return uit;
...
export function assembleerTegels({ publicaties = [], overheidDocs = [], items = [], nu = Date.now() }) {
  ...
  for (const thema of OVERHEID_THEMAS) {
    ...
    for (const { d, t } of lijst) {
      ...
      art.themaLabel = OVERHEID_THEMA_LABEL[thema] || "";
