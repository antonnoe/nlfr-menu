...
// Ligaturen die Unicode NIET zelf uit elkaar haalt. NFKD lost ﬁ en ﬂ op, maar
// œ, æ en ß hebben geen compatibiliteitsdecompositie: die blijven staan en
// vallen dan bij het splitsen weg. Gemeten in de echte index: "main d'œuvre"
// werd ["main","uvre","nodig"] — niet leeg, maar een onzinwoord dat aan een
// ander verminkt woord kan blijven haken. Twee woorden in 349 artikelen, dus
// klein, maar de reparatie is één tabel.
const LIGATUREN = [[/œ/g, "oe"], [/æ/g, "ae"], [/ß/g, "ss"], [/ø/g, "o"]];

export function tokens(tekst) {
  let t = String(tekst || "").toLowerCase();
  for (const [van, naar] of LIGATUREN) t = t.replace(van, naar);
  return t
    // NFKD, niet NFD: dat haalt ook ﬁ en ﬂ uit elkaar.
    .normalize("NFKD")
    .replace(/[̀-ͯ]/g, "")
    .split(/[^a-z0-9]+/)
