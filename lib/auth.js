...
export function schrijfCookie(name, value, options) {
  ...
  //
  // De waarde wordt genormaliseerd naar één schrijfwijze. Browsers lezen hem
  // hoofdletterongevoelig, maar @supabase/ssr levert "lax" aan en deze module
  // schrijft "Lax"; twee schrijfwijzen door elkaar maakt elke controle op deze
  // regel — in de tests, of met het oog in een devtools-venster — een gok.
  const gelijk = { lax: "Lax", strict: "Strict", none: "None" };
  delen.push(`SameSite=${gelijk[String(o.sameSite || "Lax").toLowerCase()] || "Lax"}`);
