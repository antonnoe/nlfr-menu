...
// weggegooid.
function geldigeBronUrl(kandidaten, standaardNaam = "") {
  for (const k of [kandidaten].flat()) {
    ...
    const oordeel = bronUrlOordeel(ruw, bronVoorNaam(naam) || {});
...
function schoneBronnen(bronnen, standaardNaam = "") {
  return (bronnen || []).map((b) => {
    const naam = (b && b.naam) || standaardNaam;
    const oordeel = bronUrlOordeel(b && b.url, bronVoorNaam(naam) || {});
...
export function assembleerTegels({ publicaties = [], overheidDocs = [], items = [], nu = Date.now() }) {
  ...
  if (ver.length) {
    tegels.push({
      ...
      artikelen: ver.map((i) => ({
        id: `ver-${i.url}`,
        url: geldigeBronUrl(i.url, i.bron),
        soort: "verenigingen",
        titel: i.titel,
        summary: eersteZin(i.samenvatting) || i.titel,
        tekst: i.samenvatting || "",
