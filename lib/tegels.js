...
// kandidaat een absolute http(s)-URL op, dan is die bewust null — de afnemer
// hoort dat item zonder link te tonen, niet met een dode link.
function absoluteBronUrl(...kandidaten) {
  for (const k of kandidaten.flat()) {
    const ruw = typeof k === "string" ? k : k && k.url;
    if (!ruw) continue;
    try {
      const u = new URL(String(ruw).trim());
      if (u.protocol === "http:" || u.protocol === "https:") return u.href;
    } catch {
      // Geen absolute URL (relatief pad, rommel) — volgende kandidaat.
    }
  }
  return null;
}
...
function persArtikel(pub) {
  ...
  return {
    ...
    url: absoluteBronUrl(pub.bronnen || []),
