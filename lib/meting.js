...
// Grootte van de body, in bytes. Twintig gebeurtenissen met een id van hooguit
// 80 tekens komen op een paar kilobyte; 8 KB is daar ruim boven en ver onder
// wat het platform toelaat. Zonder deze grens wordt een verzoek eerst volledig
// ingelezen en geparseerd VOORDAT GEBEURTENIS_MAX iets afkapt, en dan kost een
// body van megabytes nog steeds geheugen en rekentijd — bij een endpoint dat
// voor iedereen openstaat is dat een gratis hefboom.
...
// De laatste `aantal` dagen, nieuwste eerst.
//
// HIER ZAT EEN FOUT, en hij is leerzaam genoeg om te bewaren. De eerste versie
// telde terug in stappen van 24 uur vanaf NU en stempelde elke uitkomst in
// Parijse tijd. Dat lijkt veilig, want een UTC-etmaal is altijd precies 24 uur.
// Maar de Parijse KLOK verschuift er wel onder: op 2026-03-29T22:30Z is het in
// Frankrijk 00:30 op 30 maart, en 24 uur eerder 23:30 op 28 maart. Resultaat:
// 29 maart bestond niet in de reeks. In het najaar levert dezelfde som twee keer
// dezelfde dag, en dan kwam de reeks een dag tekort.
//
// Daarom rekent dit nu in KALENDERDAGEN: eerst de Parijse datum van nu, en dan
// dagen aftrekken van die datum zelf. Een datum zonder tijd kent geen zomertijd,
// dus hier valt niets meer te verschuiven.
export function laatsteDagen(aantal, nu, tijdzone) {
  const n = Math.max(1, Math.min(Number(aantal) || 1, 400));
  const vandaag = dagStempel(nu, tijdzone);
  if (!vandaag) return [];
  const [jaar, maand, dag] = vandaag.split("-").map(Number);
  const start = Date.UTC(jaar, maand - 1, dag);
  const uit = [];
  for (let i = 0; i < n; i += 1) {
    const d = new Date(start - i * 24 * 60 * 60 * 1000);
    const mm = String(d.getUTCMonth() + 1).padStart(2, "0");
    const dd = String(d.getUTCDate()).padStart(2, "0");
    uit.push(`${d.getUTCFullYear()}-${mm}-${dd}`);
  }
  return uit;
}
