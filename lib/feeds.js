...
// ---- Voetbal-zeef voor pers -------------------------------------------------
// True = doorlaten. Voetbal is off-topic, TENZIJ het om een ernstig incident
// in/rond een stadion gaat (geweld, drama, instorting) — dan blijft het staan.
export function voetbalDoorlaat(titel) {
  const n = normaliseer(titel);
  if (!VOETBAL_UIT.some((w) => n.includes(w))) return true; // geen voetbal
  return VOETBAL_INCIDENT_IN.some((w) => n.includes(w)); // wel voetbal -> alleen bij incident
}
