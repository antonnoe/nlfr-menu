  ...
  const bronStatus = resultaten.map((r) => {
    ...
    console.log(
      `[feeds] ${r.bron.naam}: http=${r.http}, ruw=${r.ruw}, na-filter=${r.items.length}` +
        (geweigerd.length ? `, bron-URL geweigerd=${geweigerd.length}` : "")
    );
    // Een geweigerde bron-URL is bijna altijd een structureel probleem (een
    // bron die zijn itemlink anders is gaan aanleveren), niet één rare post.
    // Daarom apart in de log én in de status, met reden — anders zou de fout
    // die deze module aanleiding gaf opnieuw onzichtbaar verdwijnen.
    for (const g of geweigerd) {
      console.warn(`[feeds] ${r.bron.naam}: bron-URL geweigerd — ${g.reden} — "${g.titel}"`);
    }
    return {
      naam: r.bron.naam,
      thema: r.bron.thema,
