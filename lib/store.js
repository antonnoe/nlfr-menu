...
// Hoeveel sleutels er per MGET meegaan. Upstash accepteert er meer, maar een
// begrensde batch houdt de request-body en het geheugen voorspelbaar; bij de
// huidige voorraad (~145 documenten) zijn dit twee round-trips in plaats van
// 145 losse GET's.
export const MGET_BATCH = 100;

// Alle JSON-documenten voor een patroon. Sleutels die tussen scan en lezen
// verlopen zijn, worden er stil uitgefilterd — net als documenten die niet te
// parsen zijn. Zonder KV-configuratie komt er een lege lijst terug.
//
// De sleutels worden in batches van MGET_BATCH tegelijk opgehaald (MGET) in
// plaats van één GET per sleutel. Dat was de tweede oorzaak van de trage
// cache-miss op /api/actueel: ~145 sequentiële round-trips naar Upstash.
export async function listJSON(patroon) {
  ...
  try {
    ...
    for (let i = 0; i < keys.length; i += MGET_BATCH) {
      const groep = keys.slice(i, i + MGET_BATCH);
      const waarden = await cmd(["MGET", ...groep]);
      if (!Array.isArray(waarden)) continue;
      for (const waarde of waarden) {
        if (waarde == null) continue; // tussen scan en lezen verlopen
        let doc;
        try {
          doc = JSON.parse(waarde);
        } catch {
          continue; // onparseerbaar document: stil overslaan
        }
        if (doc) uit.push(doc);
      }
    }
...
// Meerdere opdrachten in één HTTP-verzoek (Upstash-pipeline). Voor de meting
// scheelt dat het verschil tussen één round-trip per paginabezoek en vijf: die
// teller wordt vanuit elke pagina van nederlanders.fr aangeroepen, dus daar
// telt elke round-trip mee in tijd én in kosten.
//
// Het antwoord is een lijst met per opdracht {result} of {error}. Een fout in
// één opdracht laat de rest staan; de aanroeper krijgt de lijst zoals hij is en
// beslist zelf wat een gemiste teller waard is (voor de meting: niets — een
// verloren telling is geen reden om een bezoeker een fout te tonen).
export async function pipeline(opdrachten) {
  if (!kvBeschikbaar()) {
    throw new Error(
      "KV niet geconfigureerd (KV_REST_API_URL / KV_REST_API_TOKEN ontbreken)."
    );
  }
  const lijst = Array.isArray(opdrachten) ? opdrachten : [];
  if (!lijst.length) return [];
  const basis = String(URL_ENV).replace(/\/+$/, "");
  const res = await fetch(`${basis}/pipeline`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${TOKEN_ENV}`,
      "Content-Type": "application/json",
  ...
  const json = await res.json();
  // Een antwoord dat GEEN lijst is, is een kapot antwoord en geen leeg antwoord.
  // Stil een lege lijst teruggeven zou de aanroeper laten denken dat elke
  // opdracht netjes niets opleverde — bij een meting is dat het verschil tussen
  // "er is vandaag niet geklikt" en "de opslag antwoordt onzin", en juist dat
  // verschil moet zichtbaar blijven.
  if (!Array.isArray(json)) {
    throw new Error("KV-fout: onverwacht pipeline-antwoord (geen lijst).");
  }
  return json;
}
